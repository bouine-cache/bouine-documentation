...
| `idle_timeout` | `120s` | Keep-alive idle timeout for data-plane connections: how long a connection with no in-flight request stays open. Also used by the H1 fast-path parser, so the two stay in sync. With an nginx front-end, keep nginx's `keepalive_timeout` **below** this value so nginx closes idle connections first — otherwise bouine can close a connection mid-reuse and nginx logs `upstream prematurely closed connection`. |
...
| `max_idle_conn_duration` | `90s` | How long an idle pooled origin connection is kept before closing (nginx upstream `keepalive_timeout` equivalent). Keep this **below** any LB idle timeout between bouine and the origin (e.g. AWS NLB 350s) so bouine closes idle connections first. |
